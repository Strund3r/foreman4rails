...
#         execute "/home/deploy/.rvm/bin/rvm all do foreman export upstart /etc/init -f /home/deploy/apps/foreman4rails/current/Procfile -a #{fetch(:application)} -u #{fetch(:user)} -l #{current_path}/log -c #{!ENV['concurrency'].nil? ? ENV['concurrency'] : "#{processes.map{ |p| "#{p[:process]}=#{p[:concurrency]}"}.join(",")}"}"
