...
namespace :foreman do
  ...
  task :export do
    on roles(:app) do
        execute :exec, "#{try_sudo} bundle exec foreman export upstart /etc/init --procfile=./Procfile -a #{fetch(:application)} -u #{fetch(:user)} -l #{current_path}/log"
    end
  end
  ...
  task :start do
    on roles(:app) do
        execute :exec, "#{try_sudo} foreman start #{fetch(:application)}"
    end
  end
  ...
  task :stop do
    on roles(:app) do
        execute :exec, "#{try_sudo} foreman stop #{fetch(:application)}"
    end
  end
  ...
  task :restart do
    on roles(:app) do
        execute :exec, "#{try_sudo} foreman start #{fetch(:application)} || foreman restart #{fetch(:application)}"
    end
  end
end
